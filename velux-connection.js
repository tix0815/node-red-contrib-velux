'use strict'
var klf = require('./lib/velux-klf200/klf')
  ...
  function veluxConnectionNode(config) {
    ...
    function connected(v) {
      ...
      node.velux.event.on('error',(err)=>{
        // FORK (tix0815): zweiarmiges node.error -> vom catch-Node fangbar (Dashboard-Toast)
        node.error(util.format('Velux %s', err), {payload: String(err), source: 'velux-connection'})
      })
